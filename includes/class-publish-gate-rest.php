...
class Publish_Gate_REST {
	...
	/**
				...
				return array(
					'passed'  => $passed,
					'message' => $passed
						? sprintf( __( 'Found %d headings (minimum %d).', 'publish-gate' ), $count, $min_count )
						: sprintf( __( 'Found %d headings (minimum %d required).', 'publish-gate' ), $count, $min_count ),
				);

			case 'max_word_count':
				$max_words  = isset( $config['max_words'] ) ? absint( $config['max_words'] ) : 5000;
				$word_count = str_word_count( wp_strip_all_tags( $post->post_content ) );
				$passed     = $word_count <= $max_words;
				...
				return array(
					'passed'  => $passed,
					'message' => $passed
						? sprintf( __( 'Word count: %d (within limit of %d).', 'publish-gate' ), $word_count, $max_words )
						: sprintf( __( 'Word count: %d (maximum %d allowed).', 'publish-gate' ), $word_count, $max_words ),
				);

			case 'required_block':
				$block_name = isset( $config['block_name'] ) ? sanitize_text_field( $config['block_name'] ) : '';

				if ( empty( $block_name ) ) {
					return array( 'passed' => true, 'message' => __( 'No block type configured.', 'publish-gate' ) );
				}

				$blocks = parse_blocks( $post->post_content );
				$found  = $this->has_block_type( $blocks, $block_name );

				return array(
					'passed'  => $found,
					'message' => $found
						? sprintf( __( 'Required block "%s" found.', 'publish-gate' ), $block_name )
						: sprintf( __( 'Required block "%s" is missing.', 'publish-gate' ), $block_name ),
				);

			default:
				return array(
					'passed'  => true,
					'message' => __( 'Unknown custom rule type — skipped.', 'publish-gate' ),
				);
		}
	}

	/**
	 * Count heading blocks (H2/H3) recursively.
	 *
	 * @param array $blocks Parsed blocks.
	 * @return int Number of heading blocks.
	 */
	private function count_headings( $blocks ) {
		$count = 0;
		...
		foreach ( $blocks as $block ) {
			...
			if ( ! empty( $block['innerBlocks'] ) ) {
				$count += $this->count_headings( $block['innerBlocks'] );
			}
		}
	...
	/**
	 * Check if a specific block type exists recursively.
	 *
	 * @param array  $blocks    Parsed blocks.
	 * @param string $block_name Block name to search for.
	 * @return bool Whether the block type was found.
	 */
	private function has_block_type( $blocks, $block_name ) {
		foreach ( $blocks as $block ) {
			if ( $block['blockName'] === $block_name ) {
				return true;
			}

			if ( ! empty( $block['innerBlocks'] ) && $this->has_block_type( $block['innerBlocks'], $block_name ) ) {
				return true;
			}
		}
